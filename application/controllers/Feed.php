...
class Feed extends CI_Controller
{
	...
	public function add_post()
	{
		...
		$content = $this->common->get_user_tag_usernames($content);
		$s = $this->common->get_user_hashtag($content);
		
		$content = $s['content'];
		$tagged_users = $s['users'];
		...
		if (!empty($image_url)) {
			...
			if ($target_type == "page_profile") {
				...
				if ($album->num_rows() == 0) {
					// Create
					$albumid = $this->image_model->add_album(
						array(
							"pageid" => $targetid,
							"feed_album" => 1,
							"name" => lang("ctn_646"),
			...
			for ($i = 0; $i < $cpt; $i++) {
				...
				$config['max_size'] = 0; //  here it is 2 MB
		...
		foreach ($tagged_users as $user) {
			// Notification
			$this->feed_model->add_tagged_user(
				array(
					"userid" => $user->ID,
					"postid" => $postid
				)
		...
		foreach ($users as $user) {
			$this->feed_model->add_feed_users(
				array(
					"userid" => $user->ID,
					"postid" => $postid
				)
	...
	public function like_comment($id)
	{
		...
		if ($like->num_rows() > 0) {
			...
			$this->feed_model->update_comment(
				$id,
				array(
					"likes" => $comment->likes - 1,
				)
			...
			$this->feed_model->add_comment_like(
				array(
					"userid" => $this->user->info->ID,
					"commentid" => $id,
					"timestamp" => time()
	...
	public function post_comment_reply($id)
	{
		...
		$replyid = $this->feed_model->add_comment(
			array(
				"commentid" => $com->ID,
				"userid" => $this->user->info->ID,
				"comment" => $comment,
		...
		$ajax = $this->template->returnAjax(
			"feed/feed_comment_replies_single.php",
			array(
				"com" => $coms,
				"comment" => $com,
			)
	...
	public function save_post($id, $hash)
	{
		...
		if ($saved->num_rows() == 0) {
			// Add
			$this->feed_model->add_saved_post(
				array(
					"userid" => $this->user->info->ID,
					"postid" => $id
				)
